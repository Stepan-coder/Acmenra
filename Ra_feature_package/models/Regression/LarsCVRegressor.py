...
class LarsCVRegressor:
    def __init__(self,
        ...
        self.__default = {'fit_intercept': Param(ptype=[bool],
                          ...
                          'max_n_alphas': Param(ptype=[int],
                                                def_val=1000,
                                                def_vals=conf_params(min_val=250,
                                                                     max_val=count,
                                                                     count=count,
                                                                     ltype=int)),
                          'eps': Param(ptype=[float],
                                       def_val=np.finfo(float).eps,
                                       def_vals=[np.finfo(float).eps]),
    ...
    def __repr__(self):
        return f"'<Ra.{LarsCVRegressor.__name__} model>'"
    ...
    def fit(self,
        ...
        if grid_params and param_dict is None:
            self.model = LarsCV(**self.__grid_best_params,
                                n_jobs=n_jobs,
                                verbose=verbose)
            ...
            self.model = LarsCV(**model_params,
                                n_jobs=n_jobs,
                                verbose=verbose)
        elif not grid_params and param_dict is None:
            self.model = LarsCV(n_jobs=n_jobs,
                                verbose=verbose)
        else:
