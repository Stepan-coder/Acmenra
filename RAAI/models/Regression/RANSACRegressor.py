...
from sklearn.linear_model import RANSACRegressor as RANSACRRegressor
...
class RANSACRegressor:
    ...
    def set_params(self,
        ...
        self.__default = {'base_estimator': Param(ptype=[object, type(None)],
                                                  def_val=None,
                                                  def_vals=[None]),
                          'min_samples': Param(ptype=[int],
                                               def_val=1,
                                               def_vals=conf_params(min_val=2,
                                                                    max_val=count * 2,
                                                                    count=count,
                                                                    ltype=int)),
                          'residual_threshold': Param(ptype=[float, type(None)],
                                                      def_val=None,
                                                      def_vals=[None]),
                          'is_data_valid': Param(ptype=[callable, type(None)],
                                                 def_val=None,
                                                 def_vals=[None]),
                          'is_model_valid': Param(ptype=[callable, type(None)],
                                                  def_val=None,
                                                  def_vals=[None]),
                          'max_trials': Param(ptype=[int],
                                              def_val=100,
                                              def_vals=conf_params(min_val=10,
                                                                   max_val=count * 10,
                                                                   count=count,
                                                                   ltype=int)),
                          'max_skips': Param(ptype=[int, type(None)],
                                             def_val=np.inf,
                                             def_vals=[np.inf]),
                          'stop_n_inliers': Param(ptype=[int, type(None)],
                                                  def_val=np.inf,
                                                  def_vals=[np.inf]),
                          'stop_score': Param(ptype=[float, type(None)],
                                              def_val=np.inf,
                                              def_vals=[np.inf]),
                          'stop_probability': Param(ptype=[float],
                                                    def_val=0.99,
                          ...
                          'loss': Param(ptype=[str],
                                        def_val="absolute_loss",
                                        def_vals=["absolute_loss"])}
        self.__show = show
    ...
    def fit(self,
        ...
        if grid_params and param_dict is None:
            self.model = RANSACRRegressor(**self.__grid_best_params,
                                          random_state=13)
            ...
            self.model = RANSACRRegressor(**model_params,
                                          random_state=13)
        elif not grid_params and param_dict is None:
            self.model = RANSACRRegressor(random_state=13)
        else:
    ...
    def fit_grid(self,
        ...
        if self.__show:
            ...
            show_grid_params(params=model_params,
                             ...
                             n_jobs=grid_n_jobs)
        model = RANSACRRegressor(random_state=13)
